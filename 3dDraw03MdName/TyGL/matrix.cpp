...
{
  if( (c->matrix_mode == c->matrix_stack[0] )|| 
	  (c->matrix_mode == c->matrix_stack[1] ) )
    c->matrix_model_projection_updated = 1 ;//(c->matrix_mode<=1);
  else
...
void glopMatrixMode(GLContext *c,GLParam *p)
{
  ...
  switch(mode) {
  case GL_MODELVIEW:
    c->matrix_mode= c->matrix_stack[0];//0;
    break;
  case GL_PROJECTION:
    c->matrix_mode= c->matrix_stack[1];
    break;
  case GL_TEXTURE:
    c->matrix_mode= c->matrix_stack[2];
    break;
  default:
    assert(0);
  }
...
void glopLoadMatrix(GLContext *c,GLParam *p)
{
  ...
  m=c->matrix_mode;//c->matrix_stack_ptr[c->matrix_mode];
  q=p+1;
...
void glopLoadIdentity(GLContext *c,GLParam *p)
{
  ...
  M4::gl_M4_Id(c->matrix_mode);//c->matrix_stack_ptr[c->matrix_mode]);
...
void glopMultMatrix(GLContext *c,GLParam *p)
{
  ...
  M4::gl_M4_MulLeft(c->matrix_mode,&m);//c->matrix_stack_ptr[c->matrix_mode]
...
void glopPushMatrix(GLContext *c,GLParam *p)
{
  ...
  c->matrix_mode++;
  m = c->matrix_mode;//++c->matrix_stack_ptr[n];
...
void glopRotate(GLContext *c,GLParam *p)
{
  ...
  M4::gl_M4_MulLeft(c->matrix_mode,&m);
